...
def generate_vec():
    ...
    header_file.write("//-------------------------------------------------\n")
    header_file.write('#include "ast/type.h"\n')
    header_file.write('#include "variable.h"\n')
    header_file.write('#include "function.h"\n')
    ...
    for type, data_type, bits in zip(types, data_types, bit_widths):
        for size in range(1, 5):
            ...
            for type2, data_type2, bits2 in zip(types, data_types, bit_widths):
                ...
                setup_string += '    {}.type = Type::FullType{{ {}.name }};\n'.format(arg_name, type_name2)
                setup_string += '    Symbol::Resolved(&{})->typeSymbol = &{};\n'.format(arg_name, type_name2)
                setup_string += '    {}.name = "{}"_c;\n'.format(fun_name, data_type_name)
                setup_string += '    {}.returnType = Type::FullType {{ {}.name }};\n'.format(fun_name, type_name)
                setup_string += '    Symbol::Resolved(&{})->returnTypeSymbol = &{};\n\n'.format(fun_name, type_name)
            ...
            for ctor_idx, comb in enumerate(combinations):
                dec_func = 'extern Function {}_ctor{}'.format(type_name, ctor_idx)
                def_func = 'Function {}_ctor{}'.format(type_name, ctor_idx)
                ...
                for arg_idx, s in enumerate(comb):
                    ...
                    if s == 1:
                        ...
                        dec_func += '_{}x{}'.format(type, s)
                        def_func += '_{}x{}'.format(type, s)
                    ...
                    setup_args += '    {}_ctor{}_arg{}.type = Type::FullType {{ {}.name }};\n'.format(type_name, ctor_idx, arg_idx, arg_type_name)
                    setup_args += '    Symbol::Resolved(&{}_ctor{}_arg{})->typeSymbol = &{};\n'.format(type_name, ctor_idx, arg_idx, arg_type_name)
                dec_func += ';\n'
                
                setup_string += '    // Construct with {}\n'.format(arg_list[0:-2])
                setup_string += setup_args
                setup_string += '    {}_ctor{}.name = "{}"_c;\n'.format(type_name, ctor_idx, data_type_name)
                setup_string += '    {}_ctor{}.returnType = Type::FullType{{ {}.name }};\n'.format(type_name, ctor_idx, type_name)
                setup_string += '    Symbol::Resolved(&{}_ctor{})->returnTypeSymbol = &{};\n\n'.format(type_name, ctor_idx, type_name)
                ...
                declaration_string += '{}'.format(dec_func)
