...
def generate_types():
    ...
    spirv_intrinsics.write('{\n')
    spirv_intrinsics.write('using SPIRVConverter = [](const Compiler*, SPIRVGenerator*, uint32_t, SPIRVResult) -> SPIRVResult;\n')
    spirv_intrinsics.write('using SPIRVIntrinsic = [](const Compiler*, SPIRVGenerator*, uint32_t, const std::vector<SPIRVResult>&) -> SPIRVResult;\n')
    spirv_intrinsics.write('constexpr ConverterTable = StaticMap{ std::array{\n')
    spirv_converter = ''
    for converter in type_conversions:
        spirv_converter += '    {{ TypeConversionTable::{}, SPIRVConverter {{\n'.format(converter.enum)
    ...
    def spirv_intrinsic(fun, arg):
        return 'std::pair{{ &{} , SPIRVIntrinsic\n{{\n    {}\n}};\n'.format(fun, arg)
